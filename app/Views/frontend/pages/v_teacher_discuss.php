...
foreach ($transaction as $key => $value) { ?>
    ...
    <li class="list-group-item <?= $key == 0 ? 'open-chat' : ''; ?> navigation-chat" data-id="<?= $value['transaction_id']; ?>">
        <figure class="avatar avatar-state-success">
            <img src="/uploads/images/<?= $value['student_image']; ?>" class="rounded-circle" alt="image">
        </figure>
        <div class="users-list-body">
            <div>
                <h5 class="text-primary"><?= $value['student_name']; ?></h5>
        ...
        <div class="chat-body">
            <div class="messages">
                ...
                foreach ($value['chat'] as $k => $val) { ?>
                    <div class="message-item <?= !empty($val['teacher_id']) ? 'outgoing-message' : ''; ?>">
                        <div class="message-avatar">
                            <figure class="avatar">
                                <img src="/uploads/images/<?= !empty($val['teacher_id']) ? $value['teacher_image'] : $value['student_image']; ?>" class="rounded-circle" alt="image">
                            </figure>
                            <div>
                                <h5><?= !empty($val['teacher_id']) ? $value['teacher_name'] : $value['student_name']; ?></h5>
